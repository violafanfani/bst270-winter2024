...
```{r}
data_after_fl = data %>% select(all_columns) %>% ## 5415 columns -> 24 columns
  filter_lipid()
nrow(data_after_fl)
```
...
```{r}
data_after_fc = data_after_fl %>% filter_confounders()
nrow(data_after_fc)
```
...
### Step 4 Filter optimism variables
```{r}
data_after_fo = data_after_fp %>% filter_optimism()
nrow(data_after_fo)
```
